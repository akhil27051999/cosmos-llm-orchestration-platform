# Cosmos — The Complete Build Program (6–12 months, "learn everything")
...
## 0. The vision — what Cosmos becomes at full scope
...
- **Build:** production k8s (RBAC, NetworkPolicies, PodSecurity, PDB); **KEDA** autoscaling on **Kafka consumer lag / queue depth**; **build a custom Operator + CRD** (e.g. an `AgentJob` or `CosmosTenant` CRD); admission control (**Kyverno/OPA**); StatefulSets/operators for the data stores; **Kustomize** overlays.
